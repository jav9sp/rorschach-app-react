...
export function interpretStressControl(
  ...
  if (adjD !== null && adjD < 0) {
    interpretaciones.push(
      `Se observa que ${persona} no cuenta con una adecuada capacidad para controlar y dirigir sus conductas ante las tensiones de la vida cotidiana, por lo que se encuentra actualmente en un estado de sobrecarga, procesando mayor tensión interna de la que es capaz de manejar.`
    );
  }
  ...
  switch (ea) {
    ...
    case "Levemente por debajo":
      interpretaciones.push(
        `Pese a lo anterior, se observa que la cantidad de sus recursos se encuentra levemente disminuida, por lo que independientemente del nivel de tensión interna que ${persona} registra actualmente, se encuentra en un estado de vulnerabilidad ante situaciones de estrés, por lo que requiere de un ambiente controlado para funcionar adaptativamente.`
      );
      break;
    case "Marcadamente por debajo":
      interpretaciones.push(
        `En cuanto a sus recursos disponibles, estos se encuentran muy por debajo de lo esperado, por lo que, independientemente del nivel de tensión interna que ${persona} registra actualmente, se encuentra en un estado de vulnerabilidad crónica ante situaciones de estrés, lo que aumenta su necesidad de un ambiente controlado y previsible para funcionar adaptativamente.`
      );
      break;
  }
  ...
  if (m === 0 && sumPonC > 3) {
    interpretaciones.push(
      `Si bien cuenta con recursos para realizar intercambios emocionales, muestra una tendencia a verse abrumad${vocal} por el afecto lo que incide en su pensamiento deliberado dificultando su tarea de demorar la descarga de afectos y focalizar los procesos de atención. Esto resulta en que bajo situaciones de alta estimulación emocional incrementa considerablemente la impulsividad y labilidad de sus controles.`
    );
  }
  ...
  if (tipoVivencial === "Extroversivo") {
    interpretaciones.push(
      `Dado que ${persona} tiene un tipo vivencial ${tipoVivencial}, [COMPLETAR]`
    );
  }

  if (tipoVivencial === "Introversivo") {
    interpretaciones.push(
      `Dado que ${persona} tiene un tipo vivencial ${tipoVivencial}, responde principalmente de manera ideacional, demorando la toma de decisiones y manteniendo las emociones al margen mientras soluciona problemas.`
    );
  }

  if (tipoVivencial === "Ambigual") {
    interpretaciones.push(
      `Por otro lado, dado que ${persona} tiene un tipo vivencial ${tipoVivencial}, es particularmente vulnerable ante situaciones de estrés debido a la inconsistencia interna de su respuesta ante los estímulos del entorno, volviendo su conducta más errática e imprevisible. `
    );
  }
